...
%TODO discuss differences/similarities between parallel/concurrent/distributed?
% Maybe in its own section?

% Just some stuff I started writing that I felt is better suited for either
% intro or its own section on the differences
In concurrent programming for instance, the cost of information transfer between
processes is minimal since the transfer is done locally, whereas in distributed
programming this is much more expensive, and even potentially impossible due to
network conditions. Distributed programming also has to deal with graceful
failures as nodes go down in potentially distant locations, whereas there is no
such thing as a partial failure in a concurrent programming environment. With
